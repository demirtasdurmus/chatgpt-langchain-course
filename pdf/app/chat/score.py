...
def random_component_by_score(component_type, component_map):
    # Make sure is component type is llm, retriever or memory
    if component_type not in ['llm', 'retriever', 'memory']:
        raise ValueError('Invalid component_type')

    # From redis, get the hash containing the sum total scores for the given component
    values = client.hgetall(f'{component_type}_score_values')

    # From redis, get the hash containing the number of times each component is called
    counts = client.hgetall(f'{component_type}_score_counts')

    # Get all the valid component names from comp map
    names = component_map.keys()

    # Loop over those valid names and use them to calculate the av
